    ...
    <div className="h-12 border-b border-gray-100 flex items-center px-4 gap-2 bg-white flex-shrink-0">
      ...
      {isEditing ? (
        <input
          ...
          placeholder="输入作品标题..."
          className="text-sm font-semibold bg-literary-primary-light border border-[#d4c4b0] rounded px-2 py-0.5 outline-none min-w-[120px]"
        />
      ) : (
        <button
          ...
          className={`text-sm font-semibold transition ${
            title.startsWith("未命名")
              ? "text-gray-400 hover:text-literary-primary"
              : "text-gray-900 hover:text-literary-primary"
          }`}
        >
          {title}
        </button>
                    ...
                    {visibleItems.map((item, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {isFirst && (
                            <span className="text-[10px] text-literary-primary bg-literary-primary-light border border-[#e8ddd0] px-1.5 py-0.5 rounded">
                              最近更新
                            </span>
                          )}
                        </button>
          ...
          <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between flex-shrink-0">
            ...
            <button
              ...
              className={`w-10 h-5.5 rounded-full transition-colors relative ${
                showChanges ? "bg-literary-primary" : "bg-gray-300"
              }`}
            ...
            return (
              <button
                ...
                className={`px-3 py-2.5 text-sm transition relative ${
                  activeTab === tab.key
                    ? "text-literary-primary font-medium"
                    : available
              ...
              >
                {tab.label}
                {activeTab === tab.key && (
                  <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-literary-primary rounded-full" />
                )}
              </button>
            );
          })}
              ...
              <div key={item.label} className="flex items-start gap-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {item.value.split(" · ").map((tag) => (
                    <span key={tag} className="px-2.5 py-1 bg-literary-primary-light text-literary-primary text-sm rounded-full">{tag}</span>
                  ))}
                </div>
              </div>
            ))}
    ...
    return (
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="bg-literary-primary-light/50 rounded-lg p-4 space-y-2.5">
            <span className="text-base font-bold text-literary-primary">陆知行</span>
            {[
              ...
              <div key={item.label} className="flex gap-3">
                <span className="text-xs text-literary-primary w-10 shrink-0 pt-0.5">{item.label}</span>
                <p className="text-sm text-gray-700 leading-relaxed">{item.value}</p>
              </div>
            ))}
          </div>
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center gap-3 mb-4">
          <span className="px-3 py-1.5 bg-literary-primary-light text-literary-primary text-sm rounded-full font-medium">四季章回 · 甜中带虐</span>
