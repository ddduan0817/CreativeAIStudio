  ...
  if ((scene === "novel" || scene === "screenplay" || scene === "marketing" || scene === "knowledge") && creationStage >= 1 && creationStage <= 4) {
    ...
    const outlineData: Record<string, {
      ...
      knowledge: {
        ...
        tagColors: {
          "总览": "bg-literary-primary-light text-literary-primary border-[#e8ddd0]",
                    ...
                    <div>
                      ...
                      <div className="space-y-4 ml-1">
                        {script.scenes?.map((s: { act: number; heading: string; characters: string[]; duration: string; productExposure: string; visual: string; dialogue: string; sellingPoint?: string }, i: number) => (
                          <div key={i} className="border-l-2 border-[#d4c4b0] pl-3">
                      ...
                      <div className="space-y-4 ml-1">
                        {ls.phases?.map((p: { timeRange: string; phase: string; script: string; interaction: string; productExposure: string }, i: number) => (
                          <div key={i} className="border-l-2 border-amber-200 pl-3">
                            ...
                            <p className="text-xs text-literary-primary mb-0.5">互动：{p.interaction}</p>
                    ...
                    <div contentEditable suppressContentEditableWarning>
                      ...
                      <div className="flex flex-wrap gap-1.5 ml-1">
                        {note.tags?.map((tag: string, i: number) => (
                          <span key={i} className="text-xs px-2 py-0.5 rounded-full bg-literary-primary-light text-literary-primary">{tag}</span>
                        ))}
                      </div>
                    </div>
                  </>
                      ...
                      <div className="space-y-4 ml-1">
                        {brief.directions?.map((dir: { title: string; storyType: string; overview: string; hook: string }, i: number) => (
                          <div key={i}>
                            ...
                            <p className="text-sm text-literary-primary italic">钩子：{dir.hook}</p>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <div className="bg-white rounded-xl shadow-2xl border w-[440px] max-h-[70vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                <div className="flex justify-end">
                  <div className="bg-literary-primary-light rounded-lg px-3 py-2 max-w-[80%]">
                    <p className="text-xs text-literary-primary font-medium">{atmosphereDialog.actionLabel || "AI 调整"}</p>
              ...
              {!atmosphereDialog.generating && atmosphereDialog.result && (
                <div className="px-4 py-3 border-t border-gray-100 flex gap-4">
                  <button onClick={() => { showToast("已替换"); setAtmosphereDialog({ show: false, text: "", generating: false, result: "", actionLabel: "" }); }} className="text-sm text-gray-700 hover:text-literary-primary transition">替换原文</button>
                  <button onClick={handleCopyResult} className="text-sm text-gray-700 hover:text-literary-primary transition">复制</button>
                  <button onClick={handleRegenerate} className="text-sm text-gray-700 hover:text-literary-primary transition">换一换</button>
                </div>
              )}
            </div>
          </div>
                  ...
                  {novelChapters.map((ch, i) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 flex items-center gap-2.5 transition text-sm ${
                        i === currentNovelChapter
                          ? "bg-literary-primary-light text-literary-primary"
                    ...
                    >
                      {ch.status === "done" ? (
                        ...
                        <Loader2 className="w-3.5 h-3.5 text-literary-primary shrink-0 animate-spin" />
                      ) : (
                    ...
                    <div key={i} id={`chapter-${i}`} className="mb-12">
                      ...
                      {i === latestIdx && latestIdx > 0 && !showPreviousChapters && (
                        <button
                          ...
                          className="mb-6 text-xs text-literary-primary hover:text-literary-primary transition flex items-center gap-1"
                        >
                          <span>↑</span> 查看前面的章节
                        </button>
                      )}
                        ...
                        <div className="prose prose-sm max-w-none">
                          {isChGenerating ? (
                            <div className="text-sm text-gray-700 leading-[1.8] whitespace-pre-wrap">
                              {ch.content}
                              <span className="inline-block w-0.5 h-4 bg-literary-primary animate-pulse ml-0.5 align-middle" />
                            </div>
                          ) : (
                            <div
                              contentEditable
        ...
        <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/10">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              <div className="flex justify-end">
                <div className="bg-literary-primary-light rounded-lg px-3 py-2 max-w-[80%]">
                  <p className="text-xs text-literary-primary font-medium">
            ...
            {!atmosphereDialog.generating && atmosphereDialog.result && (
              <div className="px-4 py-3 border-t border-gray-100 flex gap-4">
                <button
                  ...
                  className="text-sm text-gray-700 hover:text-literary-primary transition"
                >
                  替换原文
                </button>
                <button
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6" ref={containerRef}>
        <div className="max-w-3xl mx-auto space-y-5">
          {characters.map((char, i) => (
            ...
            >
              {/* Card */}
              <div className="flex-1 border border-gray-200 rounded-xl transition focus-within:border-literary-primary focus-within:shadow-[0_0_0_2px_rgba(176,137,104,0.15)]">
                {/* Role dropdown - top right */}
                <div className="relative">
                  <select
                    value={char.role}
                    onChange={(e) => onUpdate(i, { ...char, role: e.target.value as "主角" | "配角" })}
                    className="absolute top-3 right-3 text-xs border border-gray-200 rounded-md px-2 py-1 focus:outline-none focus:border-[#c4aa8e] bg-white text-gray-500 cursor-pointer z-10"
                  >
              ...
              <div className="flex flex-col items-center justify-between self-stretch py-4">
                ...
                <button
                  ...
                  className="p-1.5 text-gray-300 hover:text-literary-primary hover:bg-literary-primary-light rounded-lg transition disabled:opacity-50"
                  title={char.desc ? "智能优化" : "智能填写"}
                >
                  {generatingIdx === i
                    ? <Loader2 className="w-4 h-4 animate-spin text-literary-primary" />
      ...
      <div className="px-6 py-3 border-t border-gray-100 flex items-center">
        ...
        <button
          onClick={handleAdd}
          className="flex items-center justify-center gap-1.5 px-16 py-2.5 border border-dashed border-gray-200 rounded-xl text-sm text-gray-400 hover:border-[#c4aa8e] hover:text-literary-primary hover:bg-literary-primary-light/30 transition"
        >
