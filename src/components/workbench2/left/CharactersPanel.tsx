    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={generating}
          className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs text-literary-primary bg-literary-primary-light rounded-lg hover:bg-[#ebe0d2] transition disabled:opacity-50"
        >
          {generating ? (
...
function CharCard({
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <button
            ...
            className="flex items-center gap-1 px-1.5 py-0.5 text-xs text-literary-primary hover:text-literary-primary hover:bg-literary-primary-light rounded transition disabled:opacity-50"
          >
            {isGenerating ? (
        ...
        {multiline ? (
          <textarea
            ...
            className="w-full text-xs border border-gray-200 rounded px-2 py-1.5 resize-none focus:outline-none focus:border-[#c4aa8e] placeholder:text-gray-300"
            rows={3}
          />
        ) : (
          <input
            ...
            onChange={(e) => updateCharacter({ [field]: e.target.value })}
            placeholder={placeholder}
            className="w-full text-xs border border-gray-200 rounded px-2 py-1.5 focus:outline-none focus:border-[#c4aa8e] placeholder:text-gray-300"
          />
        )}
      </div>
    );
  ...
  if (isEditing) {
    return (
      <div className="mb-3 p-3 rounded-lg border border-[#d4c4b0] bg-literary-primary-light/30">
        {/* 基础信息 */}
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div>
            ...
            <input
              ...
              className="w-full text-xs border border-gray-200 rounded px-2 py-1.5 focus:outline-none focus:border-[#c4aa8e]"
            />
          </div>
          <div>
            ...
            <select
              ...
              className="w-full text-xs border border-gray-200 rounded px-2 py-1.5 focus:outline-none focus:border-[#c4aa8e] bg-white"
            >
              ...
              <button
                ...
                className={`px-2 py-0.5 text-xs rounded-full border transition ${
                  localChar.personality.includes(tag)
                    ? "border-[#c4aa8e] bg-[#ebe0d2] text-literary-primary"
                    : "border-gray-200 text-gray-500 hover:border-[#d4c4b0]"
                }`}
              >
                {tag}
              </button>
        ...
        <div className="flex justify-between pt-2 border-t border-gray-200">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={handleSave}
              className="px-3 py-1 text-xs bg-literary-primary text-white rounded hover:bg-literary-primary-hover transition"
            >
              保存
            </button>
          </div>
  ...
  return (
    <div
      className="mb-2 p-3 rounded-lg border border-gray-100 hover:border-[#d4c4b0] transition cursor-pointer group"
      onClick={onEdit}
    >
      <div className="flex items-center gap-2">
        <div className="w-8 h-8 rounded-full bg-[#ebe0d2] flex items-center justify-center flex-shrink-0">
          <User className="w-4 h-4 text-literary-primary" />
        </div>
