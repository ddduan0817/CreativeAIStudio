        ...
        {totalPages > 1 && (
          <div className="flex items-center gap-1">
            <button
              ...
              disabled={page === 0}
              className="p-1 rounded-md text-warm-text-muted hover:text-warm-text-secondary hover:bg-literary-primary-light transition disabled:opacity-30 disabled:cursor-not-allowed"
            >
              <ChevronLeft className="w-4 h-4" />
            </button>
            <span className="text-[11px] text-warm-text-muted tabular-nums min-w-[32px] text-center">
              {page + 1}/{totalPages}
            </span>
            <button
      ...
      <div className="flex-1">
        {sorted.map((work) => (
          <div
            ...
            className="flex items-center gap-3 px-3 py-3 rounded-xl cursor-pointer hover:bg-literary-primary-light transition group"
          >
            <span className="text-lg flex-shrink-0">{work.emoji}</span>
            <div className="flex-1 min-w-0">
              <h3 className="text-sm text-warm-text truncate">{work.title}</h3>
            ...
            <div className="relative" ref={activeMenu === work.id ? menuRef : undefined}>
              <button
                ...
                className="p-1 text-warm-text-muted hover:text-warm-text-secondary opacity-0 group-hover:opacity-100 transition"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-[380px] p-6 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-bold text-warm-text">确认删除</h3>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button
                onClick={() => setShowDeleteConfirm(null)}
                className="px-4 py-2 text-sm text-warm-text-secondary border border-gray-200 rounded-lg hover:bg-literary-primary-light transition"
              >
                取消
              </button>
              <button
