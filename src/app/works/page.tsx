      ...
      <div className="flex-1 overflow-y-auto">
        <main className="max-w-[960px] mx-auto px-8 py-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            <button
              onClick={() => router.push("/")}
              className="p-1.5 text-warm-text-muted hover:text-warm-text-secondary hover:bg-literary-primary-light rounded-lg transition"
            >
              <ArrowLeft className="w-4 h-4" />
            </button>
            <h1 className="text-lg font-bold text-warm-text">我的作品</h1>
            <div className="flex-1" />
            <button
              onClick={() => router.push("/")}
              className="px-3 py-1.5 text-xs font-medium text-white bg-literary-primary rounded-lg hover:bg-literary-primary-hover transition flex items-center gap-1"
            >
              <Plus className="w-3.5 h-3.5" />
              新建作品
            </button>
            <div className="relative">
              <Search className="w-4 h-4 text-warm-text-muted absolute left-3 top-1/2 -translate-y-1/2" />
              <input
                type="text"
                placeholder="搜索作品..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="pl-9 pr-3 py-2 text-sm bg-white border border-cream-border-warm rounded-lg w-[220px] focus:outline-none focus:border-blue-300 focus:ring-1 focus:ring-blue-100 transition placeholder:text-warm-text-muted"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 mb-6">
              ...
              <button
                ...
                className={`px-4 py-1.5 text-sm rounded-full transition ${
                  activeCategory === cat.id
                    ...
                    : "bg-white text-warm-text-secondary border border-cream-border-warm hover:bg-literary-primary-light"
                }`}
              >
            ...
            <div className="relative" ref={sortRef}>
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 text-xs text-warm-text-secondary bg-white border border-cream-border-warm rounded-lg hover:bg-literary-primary-light transition"
              >
                ...
                <div className="absolute right-0 top-full mt-1 bg-white rounded-lg shadow-lg border py-1 z-10 w-28">
                  {sortOptions.map((opt) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-1.5 text-xs hover:bg-literary-primary-light ${
                        sortBy === opt.key ? "text-literary-primary font-medium" : "text-warm-text-secondary"
                      }`}
                    >
                      {opt.label}
                    </button>
          ...
          {filteredWorks.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-sm font-medium text-warm-text-secondary">还没有作品</p>
              <p className="text-xs text-warm-text-muted mt-1">开始你的第一次创作吧</p>
              <button
                onClick={() => router.push("/")}
                className="mt-4 px-4 py-2 text-sm text-white bg-literary-primary rounded-lg hover:bg-literary-primary-hover transition inline-flex items-center gap-1"
              >
                <Plus className="w-3.5 h-3.5" />
                新建作品
              </button>
            ...
            <div className="space-y-3">
              {filteredWorks.map((work) => (
                <div
                  ...
                  className={`bg-white rounded-xl border border-cream-border p-4 flex items-center gap-4 hover:shadow-sm transition-all duration-200 group cursor-pointer ${
                    deletingId === work.id ? "opacity-0 scale-95" : ""
                  }`}
                >
                  {/* Icon */}
                  <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-cream-bg to-cream-border flex items-center justify-center text-xl flex-shrink-0">
                    {work.emoji}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <span className="text-xs px-1.5 py-0.5 rounded bg-cream-border text-warm-text-secondary">
                        {work.sceneLabel}
                      </span>
                      ...
                      {menuOpen === work.id && (
                        <div className="absolute right-0 top-full mt-1 bg-white rounded-lg shadow-lg border py-1 z-10 w-44">
                          <button
                            ...
                            className="w-full text-left px-3 py-2 text-xs text-warm-text-secondary hover:bg-literary-primary-light flex items-center gap-2"
                          >
                          ...
                          <div className="relative">
                            <button
                              ...
                              className="w-full text-left px-3 py-2 text-xs text-warm-text-secondary hover:bg-literary-primary-light flex items-center justify-between"
                            >
                              <span className="flex items-center gap-2">
                                <Download className="w-3.5 h-3.5" />
                                导出
                              </span>
                              <ChevronRight className="w-3 h-3 text-warm-text-muted" />
                            </button>
                            {exportSubOpen && (
